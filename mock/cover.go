...
func AddGasConsumption(code []byte) ([]byte, error) {
	...
	if GasPackageNickName != "" {
		file.edit.Insert(file.offset(file.astFile.Name.End()),
			fmt.Sprintf("\n\nimport %s %q", GasPackageNickName, GasImportPath))
		GasPackageName = GasPackageNickName
	} else {
		file.edit.Insert(file.offset(file.astFile.Name.End()),
			fmt.Sprintf("\n\nimport %q", GasImportPath))
	}
...
func (f *File) newCounter(start, end token.Pos, numStmt int) string {
	...
	stmt := fmt.Sprintf("\n%s.%s(%d)\n", GasPackageName, GasConsumeGasFunc, numStmt)
	//stmt := atomicCounterStmt(f, fmt.Sprintf("%s.ConsumeGas(%d)", gasPackageName, len(f.blocks)))
