...
func (ctx *defaultBlockchainContext) generateObjectID(contract types.Address) core.ObjectID {
	ctx.nonce++
	hash := sha256.Sum256([]byte(fmt.Sprintf("%s-%s-%d", contract, ctx.txHash, ctx.nonce)))
...
// Call 跨合约调用
func (ctx *defaultBlockchainContext) Call(contract types.Address, function string, args ...any) ([]byte, error) {
